...
public class hideDBAdapter {
    ...
    private static final String DATABASE_NAME = "Rosegram2";
    ...
    private static final String TAG = "Rosegram_USER_TABLE_HIDES";
    ...
    public List<hideObjc> getAllItms(int HIDE_CODE) {
        return cursorToList(this.db.query(true, DATABASE_MAINTABLE, this.yek_SH_flashkart, "hideCode == '" + HIDE_CODE + "'", null, null, null, " id desc", null));
    }
    ...
    private List<hideObjc> cursorToList(Cursor cursor) {
        List<hideObjc> items = new ArrayList();
    ...
    public int isHidden (int id ){
        try {
            open();
            hideObjc item = getItm(id);
            close();


            if (item == null )
                return -1;
            else if (item.getIsHidden() == 1){
                return item.getHideCode();
            }
        }catch (Exception e){
