...
package org.zgram.messenger.voip;
...
import org.zgram.messenger.StatsController;
...
import org.zgram.ui.VoIPActivity;
import org.zgram.ui.VoIPFeedbackActivity;
import org.zgram.ui.VoIPPermissionActivity;
...
public class VoIPService extends Service implements VoIPController.ConnectionStateListener, SensorEventListener, AudioManager.OnAudioFocusChangeListener, NotificationCenter.NotificationCenterDelegate{
	...
	public void onCreate() {
		...
		try {
			...
			cpuWakelock = ((PowerManager) getSystemService(POWER_SERVICE)).newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "zgram-voip");
			cpuWakelock.acquire();
	...
	private void startRinging() {
		...
		//ringtone.play();
		SharedPreferences prefs = AndroidUtilities.getUserPrefs("Notifications", 0);
	...
	private void configureDeviceForCall() {
		...
		try{
			if(proximity!=null){
				proximityWakelock=((PowerManager) getSystemService(Context.POWER_SERVICE)).newWakeLock(PROXIMITY_SCREEN_OFF_WAKE_LOCK, "zgram-voip-prx");
