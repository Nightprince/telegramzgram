...
import org.zgram.messenger.exoplayer2.source.hls.HlsChunkSource;
...
import org.zgram.messenger.voip.VoIPService;
...
//import edit.accManager.AccountsController;
...
import edit.dbtest.NotificationTable;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public MessagesController() {
        ...
        //gelas
        SharedPreferences Prefs = ApplicationLoader.applicationContext.getSharedPreferences("gelasConfig", Activity.MODE_PRIVATE);
    ...
    public void getNewDeleteTask(final ArrayList<Integer> oldTask) {
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                MessagesStorage.getInstance().getNewTask(oldTask,0);
            }
        });
    }
...
//    }
    ...
    public boolean processUpdateArray(ArrayList<TLRPC.Update> updates, final ArrayList<TLRPC.User> usersArr, final ArrayList<TLRPC.Chat> chatsArr, boolean fromGetDifference) {
            ...
            public void run() {
                ...
                if (!updatesOnMainThread.isEmpty()) {
                    ...
                    for (int a = 0; a < updatesOnMainThread.size(); a++) {
                        ...
                        SharedPreferences specificPreferences = ApplicationLoader.applicationContext.getSharedPreferences(PreferenceManager.prefsName, 0);
                        boolean specificContact = specificPreferences.getBoolean("specific_contact", false);
                        boolean isSpecific = specificPreferences.contains("specific_c" + update.user_id);
                        ...
                        if (update instanceof TLRPC.TL_updatePrivacy) {
                            ...
                            if (update.user_id == UserConfig.getClientUserId()) {
                                ...
                                if (update.status.expires == -1) {
                                    isInvisible = true;
                                }
                                if (specificContact && isSpecific) {
                                    if (currentUser != null && currentUser.status != null && currentUser.status.expires <= 0 && MessagesController.getInstance().onlinePrivacy.containsKey(Integer.valueOf(currentUser.id))) {
                                        getOnline = true;
                                    }
                                    if (currentUser.status.expires > ConnectionsManager.getInstance().getCurrentTime()) {
                                        getOnline = true;
                                    }
                                    if (!isInvisible && getOnline) {
                                        MessagesController.PythonNotificatin(ContactsController.formatName(currentUser.first_name, currentUser.last_name) + " " + LocaleController.getString("get_online", R.string.get_online), currentUser.id);
                                    } else if (!isInvisible) {
                                        MessagesController.PythonNotificatin(ContactsController.formatName(currentUser.first_name, currentUser.last_name) + " " + LocaleController.getString("get_offline", R.string.get_offline), currentUser.id);
                                    }
                                }
                            }
                        } else if (update instanceof TLRPC.TL_updateUserName) {
                            ...
                            if (specificContact && isSpecific) {
                                MessagesController.PythonNotificatin(ContactsController.formatName(currentUser.first_name, currentUser.last_name) + " " + LocaleController.getString("changed_name", R.string.changed_name), currentUser.id);
                            } else if (update instanceof TLRPC.TL_updateUserPhoto) {
                                if (currentUser != null) {
                                ...
                                dbUsers.add(toDbUser);
                                if (specificContact && isSpecific) {
                                    MessagesController.PythonNotificatin(ContactsController.formatName(currentUser.first_name, currentUser.last_name) + " " + LocaleController.getString("changed_photo", R.string.changed_photo), currentUser.id);
                                }
                            } else if (update instanceof TLRPC.TL_updateUserPhone) {
                                if (currentUser != null) {
                                ...
                                dbUsers.add(toDbUser);
                                if (specificContact && isSpecific) {
                                    MessagesController.PythonNotificatin(ContactsController.formatName(currentUser.first_name, currentUser.last_name) + " " + LocaleController.getString("changed_phone", R.string.changed_phone), currentUser.id);
                                }
                            } else if (update instanceof TLRPC.TL_updateDialogPinned) {
        ...
        if (!markAsReadMessages.isEmpty()) {
            MessagesStorage.getInstance().markMessagesContentAsRead(markAsReadMessages,0);
        }
        if (deletedMessages.size() != 0) {
            for (int a = 0; a < deletedMessages.size(); a++) {
...
//        Collections.sort(dialogs, dialogComparator);
    ...
    public static void PythonNotificatin(String message, int user_id) {
        int num = (int) System.currentTimeMillis();
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("SpecialNotifications", 0);
        ...
        ((NotificationManager) ApplicationLoader.applicationContext.getSystemService(NotificationTable.TABLE_NAME)).notify(num, mBuilder.build());
    }
