...
package org.zgram.messenger.query;
...
import org.zgram.ui.Components.StickersArchiveAlert;
...
public class StickersQuery {
    ...
    public static void loadArchivedStickersCount(final int type, boolean cache) {
        if (cache) {
            ...
            int count = AndroidUtilities.getUserPrefs("Notifications", 0).getInt("archivedStickersCount" + type, -1);
            if (count == -1) {
                    ...
                    AndroidUtilities.runOnUIThread(new Runnable() {
                        ...
                        public void run() {
                            if (error == null) {
                                ...
                                //SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("Notifications", Activity.MODE_PRIVATE);
                                AndroidUtilities.getUserPrefs("Notifications", 0).edit().putInt("archivedStickersCount" + type, res.count).apply();
                                NotificationCenter.getInstance().postNotificationName(NotificationCenter.archivedStickersCountDidLoaded, type);
                            }
                        }
                    });
