...
public class WebFrameLayout extends FrameLayout {
    ...
    private final String youtubeFrame = "<!DOCTYPE html><html><head><style>" +
            ...
            "        <div id=\"player\"></div>" +
            "    </div>" +
            "    <script src=\"https://www.youtube.messenger/iframe_api\"></script>" +
            "    <script>" +
            "    var player;" +
            "    YT.ready(function() {" +
    ...
    public WebFrameLayout(Context context, final BottomSheet parentDialog, String title, String descripton, String originalUrl, final String url, int w, int h) {
        ...
        parentDialog.setDelegate(new BottomSheet.BottomSheetDelegate() {
            ...
            public void onOpenAnimationEnd() {
                ...
                args.put("Referer", "http://youtube.messenger");
                boolean ok = false;
                try {
                    Uri uri = Uri.parse(openUrl);
                    String host = uri.getHost().toLowerCase();
                    if (host != null && host.endsWith("youtube.messenger") || host.endsWith("youtu.be")) {
                        ...
                        if (id != null) {
                            ok = true;
                            webView.loadDataWithBaseURL("http://youtube.messenger", String.format(youtubeFrame, id), "text/html", "UTF-8", "http://youtube.messenger");
                        }
                    }
                } catch (Exception e) {
                    FileLog.e( e);
