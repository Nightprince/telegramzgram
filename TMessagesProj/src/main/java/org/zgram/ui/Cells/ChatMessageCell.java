...
import org.zgram.ui.Components.LinkPath;
import org.zgram.ui.Components.RadialProgress;
import org.zgram.ui.Components.RoundVideoPlayingDrawable;
import org.zgram.ui.Components.SeekBar;
import org.zgram.ui.Components.SeekBarWaveform;
import org.zgram.ui.Components.StaticLayoutEx;
...
import org.zgram.ui.SecretMediaViewer;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
                ...
                if (hasLinkPreview || hasGamePreview || hasInvoicePreview) {
                    ...
                    if (documentAttachType != DOCUMENT_ATTACH_TYPE_MUSIC && documentAttachType != DOCUMENT_ATTACH_TYPE_AUDIO && documentAttachType != DOCUMENT_ATTACH_TYPE_DOCUMENT) {
                        ...
                        if (hasInvoicePreview) {
                            ...
                            stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface("fonts/iransans.ttf")), 0, price.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        if (authorName || viaBot) {
            ...
            if (viaBot) {
                ...
                if (currentNameString.length() > 0) {
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface("fonts/iransans.ttf"), 0, color), nameStringFinal.length() + 5, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface("fonts/iransans.ttf"), 0, color), 4, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    nameStringFinal = stringBuilder;
                }
        ...
        if (drawForwardedName && messageObject.isForwarded()) {
            ...
            if (currentForwardUser != null || currentForwardChannel != null) {
                ...
                if (viaString != null) {
                    ...
                    stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface("fonts/iransans.ttf")), stringBuilder.length() - viaUsername.length() - 1, stringBuilder.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                } else {
                    stringBuilder = new SpannableStringBuilder(String.format("%s %s", fromString, name));
                }
                stringBuilder.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface("fonts/iransans.ttf")), fromString.length() + 1, fromString.length() + 1 + name.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    public void updateMarker(){
        ...
        if (drawMarker) {
            drawMarker = false ;
            setDrawableBounds(Theme.markerDrawable, currentMessageObject.isOutOwner() ? currentBackgroundDrawable.getBounds().left + (Theme.markerDrawable.getIntrinsicWidth() / DOCUMENT_ATTACH_TYPE_GIF) : currentBackgroundDrawable.getBounds().right - (Theme.markerDrawable.getIntrinsicWidth() + (Theme.markerDrawable.getIntrinsicWidth() / DOCUMENT_ATTACH_TYPE_GIF)), (int) DOCUMENT_ATTACH_TYPE_NONE);
            Theme.markerDrawable.draw(mCanvas);
